...
* 在数学中：
  ...
  * 但是，这里我们从**几何视角**看待它：使用来表示空间中**以原点为起点**，以$ \begin{bmatrix} -2  \\  3  \end{bmatrix} $该点为终点的一个箭头（与物理视角不同，他是有固定起点的），他表示，从原点出发，先沿着x轴前进1个单位长度，再接着像y轴方向前进2个单位长度。
...
* r(A)<0：从矩阵变换角度看，由于$|A|=0$，变换发生了压缩，没有唯一的逆变换。同时**这个变换$A$把任意向量$x$压缩到了一个新的低维空间**。
  * 有无数解

...
一个极受欢迎的 Python 数据处理库，为Python提供了结构化的数据结构Series、DataFrame。是数据处理里面经常使用的库，可以把数据组织成表格的形式。**深度学习课程中使用的并不多**。参考：[10 分钟讲解 Pandas](http://pandas.pydata.org/pandas-docs/stable/10min.html#min)。
...
- scikit learn：一套高层次**通用**机器学习API，是综合性 Python 机器学习库（API也很经典，其他的库都有类似的API），**提供了传统的机器学习方法，功能全面**，后端可以选择Tensorflow。由于包含很多传统机器学习方法，可以与其他库配合使用。参考[Scikit-learn 官方教程](http://scikit-learn.org/stable/tutorial/basic/tutorial.html)
